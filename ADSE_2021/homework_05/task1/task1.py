...
def process_step(x_in: np.ndarray, dt: float) -> np.ndarray:
    ...
    if np.abs(x_in[4]) < 1e-3:  # Driving straight
        ...
        # input: [x-pos, y-pos, yaw, velocity, yawRate, acceleration]
        # output: [x-pos, y-pos, yaw, velocity, yawRate, acceleration]
        x = x_in[0]
        y = x_in[1]
        yaw = x_in[2]
        velocity = x_in[3]
        ...
        x_pos = x + velocity / yawRate * (np.cos(yawRate * dt + yaw) - np.cos(yaw))
        y_pos = y + velocity / yawRate * (np.sin(yawRate * dt + yaw) - np.sin(yaw))
        yaw = yaw + yawRate * dt
