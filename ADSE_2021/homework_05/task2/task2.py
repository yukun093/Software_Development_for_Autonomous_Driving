...
class VelocityCalculator:
    ...
    def __call__(self, sample=None, train_flag=False):
        ...
        for n in range(n_batch):
            ...
            x_obj = previous_positions[:, :, 0]  # Dimension: (hist_len,)
            x_obj = np.linalg.norm(x_obj, axis=1)
            y_obj = previous_positions[:, :, 1]  # Dimension: (hist_len,)
            y_obj = np.linalg.norm(y_obj, axis=1)
            ...
            # ToDo: Uncomment the following lines and derive the desired values
            xy_obj = np.stack((x_obj, y_obj), axis=-1)  # Dimension: (30, 2)
            xy_obj = np.diff(xy_obj, axis=0)
            xy_obj = np.linalg.norm(xy_obj, axis=1)
            xy_obj = np.insert(xy_obj, 0, values=0.0, axis=0)
            s_obj = np.cumsum(xy_obj)  # Dimension: (29,) --> (30,)
            dt_obj = np.diff(t_obj)
            dt_obj = np.mean(dt_obj)   # float
