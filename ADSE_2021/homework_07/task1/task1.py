...
def get_aabb_polygon(trajectory: dict) -> Polygon:
    ...
    ########################
    #  Start of your code  #
    aabb = shapely.ops.unary_union(trajectory['vehicle_polygons'])
    aabb = aabb.envelope
    ########################
    #   End of your code   #
    ########################
...
def calculate_mbc_radius(rectangle_width: float, rectangle_length: float) -> float:
    ...
    #  Start of your code  #
    ########################
    radius = np.sqrt((rectangle_width / 2) ** 2 + (rectangle_length / 2) ** 2)
    ########################
    #   End of your code   #
    ########################
...
def check_circle_collision(xy1: np.ndarray, xy2: np.ndarray, r: float) -> bool:
    ...
    #     distance = np.sqrt((xy1[i, 0] - xy2[i, 0]) ** 2 + (xy1[i, 1] - xy2[i, 1]) ** 2)  # distance between x and y
    #     d = 2 * r  # distance between two circles
    #     if distance <= d:
    #         flag = flag + 1
    # if flag > 0:
    ...
    flag = 0
    circle_collision = False
    distance = np.sqrt((xy1[0] - xy2[0]) ** 2 + (xy1[1] - xy2[1]) ** 2)  # distance between x and y
    d = 2 * r  # distance between two circles
    if distance <= d:
        flag = flag + 1
    if flag > 0:
...
def polygon_collision_check(trajectory1: dict, trajectory2: dict) -> bool:
    ...
    #  Start of your code  #
    ########################
    a = get_aabb_polygon(trajectory1)
    b = get_aabb_polygon(trajectory2)
    ########################
    # how to exactly check collision of two cars ?
    ########################
    if a.touches(b):
...
def mbc_collision_check(trajectory1: dict, trajectory2: dict, mbc_radius: float) -> bool:
    ...
    xy1 = trajectory1['xy']
    xy2 = trajectory2['xy']
    # flag = check_circle_collision(xy1, xy2, mbc_radius)
    flag = 0
    circle_collision = False
    for i in range(xy1.shape[0]):
        distance = np.sqrt((xy1[i, 0] - xy2[i, 0]) ** 2 + (xy1[i, 1] - xy2[i, 1]) ** 2)  # distance between x and y
        d = 2 * mbc_radius  # distance between two circles
        if distance <= d:
            flag = flag + 1
    if flag > 0:
        circle_collision = True
...
def hierarchical_collision_check(traj1: dict, aabb1: Polygon, traj2: dict, aabb2: Polygon, mbc_radius: float) -> tuple:
    ...
    ########################
    ########################
    # question: -is this framework correct or not ?
    ########################
    collision = False
