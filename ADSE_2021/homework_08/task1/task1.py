...
def solve_kinematic_bicycle(x0, delta_rad, v_mps):
    ...
    # Hint: x0 is given in form of a vector with [p1, p2, psi] = [0, 10, 1]
    ########################
    #  Start of your code  #
    ########################
    p1[0] = x0[0]
    ...
    for i in range(1, p1.shape[0]):
        ...
        p1[i] = p1[i - 1] + tS * v_mps[i - 1] * np.cos(psi[i - 1] + beta_delta)
        p2[i] = p2[i - 1] + tS * v_mps[i - 1] * np.sin(psi[i - 1] + beta_delta)
