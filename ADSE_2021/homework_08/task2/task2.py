...
def control(d_m, d_m_old, v_mps, radius_m, tS_s, lf_m, lr_m):
    ...
    # how to use radius to compute steering. Without radius in equation, the result more than 0.17m ?
    Kp = 0.01  # adjust the y-coordinate
    K1 = 150
    K2 = 5.0
    d_dot = (d_m - d_m_old) / tS_s
    Kc = Kp - (K1 * d_m + K2 * d_dot) / (v_mps ** 2)
