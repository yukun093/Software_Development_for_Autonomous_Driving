...
def check_trajectory(ttc, a_comb, bound_collision, ttc_treshold=2.0, a_comb_threshold=15.0):
    ...
    for i in range(a_comb.shape[0]):
        if a_comb[i] > a_comb_threshold:
            flag += 1
    if flag > 0:
        safe = False
    ########################
...
def check_bound_collision(path, bound, veh_width=2.8):
    ...
    # raise ValueError("The check_bound_collision function was not copied yet! Copy your function from task 2 over.")
    paths = shapely.geometry.LineString(path)
    bounds = shapely.geometry.LineString(bound)
    path = paths.buffer(veh_width / 2)
    bound = bounds.buffer(veh_width / 2)
    intersection = path.intersects(bound)
    ########################
    #   End of your code   #
    ########################
