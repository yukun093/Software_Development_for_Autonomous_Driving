...
def get_params() -> dict:
    ...
    params["initial_state"] = json.loads(s=parser.get(section="PARAMS", option="initial_state"))

    # read discretization from file
    params["discretization"] = json.loads(s=parser.get(section="PARAMS", option="discretization"))

    # read limits from file
    params["limits"] = json.loads(s=parser.get(section="PARAMS", option="limits"))

    # read cost coefficients from file
    params["cost_coefficients"] = json.loads(s=parser.get(section="PARAMS", option="cost_coefficients"))
