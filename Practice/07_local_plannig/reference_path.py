...
class ReferencePath:
    ...
    def __calc_sxy_rp(self, cubic_spline: CubicSpline):
        ...
        n_x = (np.ceil((cubic_spline.x[-1] - cubic_spline.x[0]) / x_sampling_size) + 1).astype(int)
        ...
        self.__xy_rp = np.column_stack((np.interp(x=self.__s_rp, xp=s_x, fp=x),
    ...
    def __calc_dkappa_rp(self, cubic_spline: CubicSpline):
        ...
        self.__dkappa_rp = ((1 + dy_rp ** 2) * dddy_rp - 3 * dy_rp * ddy_rp ** 2) / ((1 + dy_rp ** 2) ** 3)
