...
def check_collision(trajectory: Trajectory, parameters: dict) -> bool:
    ...
    for xy_obstacle in parameters["xy_obstacles"]:
        ...
        d = np.sqrt((trajectory.xy[:, 0] - xy_obstacle[0]) ** 2 + (trajectory.xy[:, 1] - xy_obstacle[1]) ** 2)

        # calculate safety factor
        safety_factor = parameters["discretization"]["max_safety_factor"] * np.ones(shape=np.size(d))

        # increase safety factor linearly by time
        slope = (parameters["discretization"]["max_safety_factor"] -
                 parameters["discretization"]["min_safety_factor"]) / \
                parameters["discretization"]["safety_factor_time"]

        safety_factor[trajectory.t <= parameters["discretization"]["safety_factor_time"]] = \
            slope * trajectory.t[trajectory.t <= parameters["discretization"]["safety_factor_time"]] + \
            parameters["discretization"]["min_safety_factor"]

        # check if any trajectory point is too close to the object using the car's MBC radius
        collision = np.any(a=d <= safety_factor * np.sqrt((parameters["discretization"]["car_length"] / 2) ** 2 +
...
def transform_obstacles_frenet_to_cartesian(reference_path: ReferencePath, sd_obstacles: np.array) -> np.array:
    ...
    xy_rp = np.column_stack((np.interp(x=sd_obstacles[:, 0], xp=reference_path.s_rp, fp=reference_path.xy_rp[:, 0]),
                             np.interp(x=sd_obstacles[:, 0], xp=reference_path.s_rp, fp=reference_path.xy_rp[:, 1])))

    # get heading along reference path corresponding to s-values
    theta_rp = np.interp(x=sd_obstacles[:, 0], xp=reference_path.s_rp, fp=reference_path.theta_rp)

    # transform position of obstacles into cartesian coordinate system
    xy_obstacles = np.column_stack((xy_rp[:, 0] - sd_obstacles[:, 1] * np.sin(theta_rp),
                                    xy_rp[:, 1] + sd_obstacles[:, 1] * np.cos(theta_rp)))

    return xy_obstacles


def transform_state_cartesian_to_frenet(cartesian_state: dict, reference_path: ReferencePath) -> dict:
    ...
    dist = np.sqrt((cartesian_state["x"] - reference_path.xy_rp[:, 0]) ** 2 +
    ...
    frenet_state["s_dot"] = (cartesian_state["v"] * np.cos(delta_theta)) / (1 - kappa_rp * frenet_state["d"])

    d_prime = (1 - kappa_rp * frenet_state["d"]) * np.tan(delta_theta)
    frenet_state["d_dot"] = frenet_state["s_dot"] * d_prime

    frenet_state["s_ddot"] = np.cos(delta_theta) / (1 - kappa_rp * frenet_state["d"]) * (cartesian_state["a"]
                            - (frenet_state["s_dot"] ** 2) / np.cos(delta_theta)
                            * ((1 - kappa_rp * frenet_state["d"]) * np.tan(delta_theta)
                            * (cartesian_state["kappa"] * (1 - kappa_rp * frenet_state["d"])
                            / np.cos(delta_theta) - kappa_rp) - (dkappa_rp * frenet_state["d"] + kappa_rp * d_prime)))

    d_pprime = - (dkappa_rp * frenet_state["d"] + kappa_rp * d_prime) * np.tan(delta_theta) \
               + (1 - kappa_rp * frenet_state["d"]) / (np.cos(delta_theta) ** 2) \
               * (cartesian_state["kappa"] * (1 - kappa_rp * frenet_state["d"]) / np.cos(delta_theta) - kappa_rp)
    frenet_state["d_ddot"] = d_pprime * frenet_state["s_dot"] ** 2 + d_prime * frenet_state["s_ddot"]
