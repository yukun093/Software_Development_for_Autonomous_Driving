...
def main():
    ...
    for n_traj in range(1, N_TRAJECTORY+1):
        rtv = collect_Data(env, policy, ENV_NAME, None, N_TIME, True, sdir_traj=SAVE_DIR + "trajectory/", sdir_reward=SAVE_DIR + "reward/")
        print(rtv)
        summary.append(rtv)
        if IS_VIDEO:
    ...
    example_trajectorie = pd.DataFrame(pd.read_pickle(sampled_trajectories[random.randint(1,len(summary))]))
