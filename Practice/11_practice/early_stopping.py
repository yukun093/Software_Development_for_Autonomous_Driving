...
class EarlyStopping(object):
    ...
    def __call__(self, res):
        ...
        if self.is_active:
            if "patience" in self.mode:
                if (self.optval is None) or (self.is_min and res < self.optval) or (not self.is_min and res > self.optval):
                    self.optval = res
                ...
                if self.count >= self.length:
                    if (self.is_min and val > self.oldval) or (not self.is_min and val < self.oldval):
                        rtv = True
                self.oldval = val
            self.count += 1
        return rtv
