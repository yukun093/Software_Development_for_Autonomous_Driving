...
class Xception(nn.Module):
    def __init__(self, i_dim, out_channels, kernel_size, stride, transpose):
        ...
        if pointfirst:
            ...
            cnns.append(cnn(out_channels, out_channels, kernel_size=kernel_size, stride=stride, groups=out_channels))
        else:
            # depthwise -> pointwise
            cnns.append(cnn(in_channels, in_channels, kernel_size=kernel_size, stride=stride, groups=in_channels))
            cnns.append(cnn(in_channels, out_channels, kernel_size=1, stride=1))
...
class CNNs(nn.Module):
    def __init__(self, s_dim, channels=[8, 16, 32, 64, 128], kernel=5, stride=2, a_name="relu"):
        ...
        for c in channels:
            ...
            self.kernels.append(kernel - ((i_dim[1] - kernel) % stride))   # assume: H and W are the same size
...
class DCNNs(nn.Module):
    def __init__(self, i_dim, o_dim, cnn):
        ...
        if i_dim != np.prod(self.i_dim):
            print("the input size maybe wrong! in: {}, expect: {}".format(i_dim, np.prod(self.i_dim)))
            sys.exit()
        layers = []
        norms = []
        for i in range(len(cnn.kernels) - 1, 0, -1):
            layers.append(Xception(cnn.deep_net[i].o_dim, cnn.deep_net[i-1].o_dim[0], cnn.kernels[i], cnn.stride, True))
            norms.append(nn.LayerNorm(layers[-1].o_dim, elementwise_affine=False))
        self.deep_net = nn.ModuleList(layers)
        self.deep_norm = nn.ModuleList(norms)
        self.recon = Xception(cnn.deep_net[0].o_dim, o_dim[0], cnn.kernels[0], cnn.stride, True)
        self.o_dim = self.recon.o_dim
        if np.prod(o_dim) != np.prod(self.o_dim):
            print("the output size maybe wrong! out: {}, expect: {}".format(self.o_dim, o_dim))
            sys.exit()
        #
