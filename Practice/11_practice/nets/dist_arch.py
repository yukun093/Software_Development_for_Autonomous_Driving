...
class PolicyLaplace(nn.Module):
    ...
    def forward(self, x):
        return torch.distributions.laplace.Laplace(self.loc(x), torch.exp(self.scale(x)))
...
class PolicyBeta(nn.Module):
    ...
    def forward(self, x):
        return torch.distributions.beta.Beta(F.softplus(self.c1(x)) + self.offset, F.softplus(self.c0(x)) + self.offset)
