...
def get_EnvInfo(env):
    s_dim = env.observation_space.shape[0] if len(env.observation_space.shape) == 1 else (1, 32, 32)
    a_dim = env.action_space.shape[0]
    trans = None if isinstance(s_dim, int) else transforms.Compose([transforms.ToPILImage(), transforms.Resize((32,32)), transforms.Grayscale(), transforms.ToTensor()])
    return s_dim, a_dim, trans


def init_Policy(env, policy, scale_factor=2.0, dataset=None, n_epoch=10, n_sample=256, n_count=100):
    # prepare prior
    if "laplace" in policy.dist:
        try:
            loc = torch.from_numpy((0.5 * (env.action_space.high + env.action_space.low)).astype(np.float32)).to(policy.device)
            scale = torch.from_numpy((0.5 * (env.action_space.high - env.action_space.low)).astype(np.float32)).to(policy.device)
        except:
        ...
        for epch in range(1, n_epoch+1):
            ...
            if dataset is not None:
                ...
                for _ in range(n_count):
                    so = np.array([env.observation_space.sample().tolist() for _ in range(n_sample)])
                    loss_sum += _init_policy(so)
                    loss_num += 1
            print("{}-th epoch to initialize policy was end: \n\tloss = {}".format(epch, loss_sum / loss_num))

######################################################
def collect_Data(env, policy, env_name, transform, n_time, is_view, sdir_traj=None, sdir_reward=None, sdir_action=None, file_name=None):
    trajectory = []
    result = []
    actions = []
    ...
    if sdir_reward is not None:
        np.savetxt(sdir_reward + file_name + ".csv", np.array([result]).T, delimiter=",")
